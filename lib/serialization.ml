...
type 'a t = { sexp_of : 'a -> Sexp.t; of_sexp : Sexp.t -> 'a }

let create_m (type a) (module M : Sexpable.S with type t = a) : a t =
...
(* Because sexp serialization is an intensional property of a type -- syntactic
   details like label names matter -- we cannot simply use the [Of_basic]
   functor. Instead, we use the [Types] and [Fold] functors from
   [Ppx_derive_at_runtime_lib].

   When constructing a function using a [Fold] functor, we do the folding
   outside [fun] or [function]. This way we traverse the type only once. *)
module Derive = struct
  ...
  (* The [Types] functors gives us the GADT binary trees for tuples, records,
     variants, and polymorphic variants. *)
  module Types = Types (Value)
  open Types

  (* We derive [sexp_of] and [of_sexp] separately, and combine them into a [t]
     later. This makes for easier to read code than trying to derive both at
     once. *)

  (* How to derive [sexp_of]: *)
  module Sexp_of = struct
    module Product_acc = struct
      (* For product types, our accumulator function conses onto a list of
         sexps. *)
    ...
    (* Conses tuple elements onto a sexp. Used for tuple values, and for inline
       tuples in variant constructor arguments. *)
    let tuple_sexps =
      Fold_tuple.fold
        ~leaf:
          { on_leaf =
    ...
    (* Conses record labels, wrapped with their labels, onto a sexp. Used for
       record values, and for inline records in variant constructor
       arguments. *)
    let record_sexps =
      Fold_record.fold
        ~leaf:
    ...
    (* Dispatches on variant structure to construct a sexp. Uses [tuple_sexps]
       and [record_sexps] from above. [on_leaf] callbacks usually need type
       annotations to guarantee they are polymorphic in the ['part] type
       parameters. *)
    let variant (Variant.T root) =
      let sexp_of =
        Fold_variant.fold root.tree
          ~leaf:
            { on_leaf =
                (fun (type part)
                   ({ name; attribute; args; create = _ } :
                     (_, part) Variant.Constructor.t) : (part -> Sexp.t) ->
                  let name =
                    match attribute with
                    | Some (Named name) -> name
                    | None -> name in
    ...
    (* Dispatches on polymorphic variant structure to construct a sexp.
       [on_leaf] callbacks usually need type annotations to guarantee they are
       polymorphic in the ['part] and ['name] type parameters. *)
    let poly_variant (Poly_variant.T root) =
      let sexp_of =
        Fold_poly_variant.fold root.tree
          ~leaf:
            { on_leaf =
                (fun (type part)
  ...
  module Of_sexp = struct
    module Product_acc = struct
      (* For product types, our accumulator function parses a prefix of a sexp
         list and returns the result plus remaining sexps. *)
    ...
    (* Most of tuple and record parsing is the same, so we share some
       helpers. *)
    ...
    let tuple (Tuple.T root) =
      let of_sexps =
        Fold_tuple.fold root.tree
          ...
          ~node:{ on_node = on_product_node } in
      product_of_sexp of_sexps root.convert

    let record (Record.T root) =
      let of_sexps =
        Fold_record.fold root.tree
          ~leaf:
            { on_leaf =
                (fun { name; attribute; value; access = _ } sexps ->
                  let name =
                    ...
                    | None -> name in
                  match sexps with
                  | List [ Atom atom; sexp ] :: sexps
                    when String.equal atom name ->
    ...
    module Sum_acc = struct
      (* For sum types, our accumulator function parses a sexp to a value, or
    ...
    (* Variant and polymorphic variant "leaf type" parsing is nontrivial, so we
       define them outside the folds. *)

    let on_variant_leaf (type a b)
    ...
    let on_poly_variant_leaf (type a b)
      ({ arg; create } : (a, b) Poly_variant.Row.t) : Sexp.t -> a option =
      match arg with
      | Empty { name; attribute } -> (
        let name =
          match attribute with
    ...
    (* Other details of sum type parsing are shared, so we can define some
       helpers. *)
  ...
  (* After defining [sexp_of] and [of_sexp], the rest is relatively
     straightforward. *)
