...
type 'a t = { compare : 'a -> 'a -> int; equal : 'a -> 'a -> bool }
...
(* Because [equal] and [compare] are extensional properties of a type -- they
   only care about the contents of values, not syntax such as field names -- we
   can derive [t] using the [Of_basic] functor. *)
include Ppx_derive_at_runtime_lib.Of_basic (struct
  ...
  let both (type a b) { compare = compare_a; equal = equal_a }
